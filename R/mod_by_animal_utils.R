...
fct_plot_by_animal <- function(df,
  ...
  p <- df %>%
    ggplot2::ggplot(aes(x = {{ col_duration }}, y = {{ col_intake }}, text = paste("animal_id:", {{ col_animal_id }})))+
  ...
  p3 <- p2 +
    ...
    ggplot2::labs(
      title = df %>% dplyr::pull({{ col_animal_id }}) %>% unique(),
...
fct_plot_animal_bins <-  function(df,
  ...
  p <-
    df  %>%
    dplyr::arrange(.data$bin_id) %>% # ggiraph needs this
    dplyr::mutate(bin_id = forcats::fct_inorder(as.factor(.data$bin_id)),
                  ...
                  duration_sec_min = round(.data$duration_sec/60, 0),
                  hover_col = glue::glue("
                                         Corrected intake (kg): {final_intake_kg}
                                         Corrected duration (sec): {final_duration_sec}
                                         Corrected duration (min): {final_duration_min}
                                         Original duration (sec): {duration_sec}
                                         Original duration (min): {duration_sec_min}\n
